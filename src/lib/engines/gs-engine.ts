 ...
 */
// gs.mjs は UMD グローバル(globalThis.exports.Module)に依存し、Rollup 本番バンドルで
// `createModule is not defined` になる。CJS 本体 gs.js を直接 default import してファクトリを
// 得ることで dev/build の両方で動作させる（返り値は Emscripten モジュール生成関数）。
import createGsModule from "@jspawn/ghostscript-wasm/gs.js";
...
const createGs: (opts?: Record<string, unknown>) => Promise<any> =
  (createGsModule as any).default ?? (createGsModule as any);
