...
// pdfcpu.wasm は 30MB で Cloudflare の 25MiB/ファイル上限を超えるため、アプリに同梱せず
// jsdelivr(npm ミラー) から版固定で取得する。プライバシー上、これは公開バイナリのDLのみで
// ユーザーのPDFは送信されない（ADR-001/004）。サプライチェーン対策として取得後に SHA-256 を検証する。
const PDFCPU_WASM_URL = "https://cdn.jsdelivr.net/npm/pdfcpu-wasm@0.1.0/pdfcpu.wasm";
const PDFCPU_WASM_SHA256 = "af8df1df621e3721778a4bddca4b1b114aecfda8f3fb3364d87c9d0a8e490054";
...
export class PdfcpuEngine implements BrowserEngine {
  ...
  async load(): Promise<LoadResult> {
    const { bytes, loadMs, fromCache } = await loadWasmBytes("pdfcpu-wasm", PDFCPU_WASM_URL, PDFCPU_WASM_SHA256);
  ...
  /**
   * pdfcpu の任意コマンドを実行し stdout/stderr を捕捉する（info/images list 等の inspect 用）。
   * シムは fd1/fd2 を console.log/error に流すため、実行中だけ console を差し替えて回収する。
   * info/images list はファイルを書かないので出力は stdout から得る。
   */
  async runCommand(
    args: string[],
    input: Uint8Array,
    ...
    let exitCode = 0;
    try {
      await p.run(args, [new File([input.slice()], "in.pdf")]);
    } catch (e) {
      exitCode = 1;
      err.push(String(e));
    } finally {
